...
func LoginUser(c *gin.Context) {
	...
	id, result := user.Login(username, password)
	...
	if result {
		...
		// 生成 token string
		tokenStr, tokenErr := middlerware.GenerateToken(userId, expireTime)
...
func ModPhoto(c *gin.Context) {
	...
	result := users.ModUser(sid, "", "", "", fileName, "")
	if result {
		c.JSON(http.StatusOK, us)
		return
	}
...
func ModUser(c *gin.Context) {
	c.Writer.Header().Add("Access-Control-Allow-Origin", "*")
	c.Next()
	tools.AddAudit(c, "更新用户信息", "进行更新用户信息")
	var sid uint64 = 0
	k, _ := c.Get("userId")
	val, ok := k.(uint64)
	if !ok {
	...
	userModReq := UserModReq{}
	err := c.BindJSON(&userModReq)
	if err != nil {
		log.Println(err)
		c.JSON(http.StatusOK, gin.H{
			"ok":  0,
	...
	user := &model.User{}
	username := userModReq.Username
	password := userModReq.Password
	phone := userModReq.Phone
	photo := userModReq.Photo
	email := userModReq.Email
	result := user.ModUser(sid, username, password, phone, photo, email)
	if !result {
		c.JSON(http.StatusOK, gin.H{
			"ok":  0,
			"msg": "用户更新失败",
